...
class ShortTermMemoryTest {
    ...
    class EdgeCases {
        @Test
        @DisplayName("Should handle null message (if not prevented by type system or validation)")
        void shouldHandleNullMessage() {
            // ShortTermMemory doesn't currently check for null messages in addMessage
            // This test documents current behavior or ensures it doesn't crash
            assertDoesNotThrow(() -> shortTermMemory.addMessage("C1", null));
            List<Message> history = shortTermMemory.getHistory("C1");
