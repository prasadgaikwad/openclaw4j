...
| **Identity/Profile** | `profiles/*.md` | Agent soul, user persona, environment facts | Permanent |
...
- **Security:** The `memory/` directory is excluded from `.git` to prevent leaking private context.
...
### 3. Agent Self-Curation
The `MemoryTool` allows the agent to decide what is "worth remembering". Instead of logging every tiny detail to `MEMORY.md`, the agent uses its reasoning to distill important facts.
- **`remember(fact)`**: Curates important information.
...
- **Context Window Blowup:** As `MEMORY.md` grows, we will need the Semantic Retrieval (RAG) planned for Slice 5.
- **Concurrency:** Simple file appending is fine for single-user localized use cases, but might need locks for high-concurrency environments.
